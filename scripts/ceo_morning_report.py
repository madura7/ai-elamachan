...
(auto-approve-eligible vs escalate-to-CEO), then:
  ...
  1. Upserts a `morning-report` document on the report host issue (VER-94).
  2. Posts a comment on VER-94 with the day's headline numbers (CEO notification).
...
Usage:
  ...
  --report-issue-id  : override report host (default: VER-94)
"""
...
DEFAULT_REPORT_ISSUE_ID = "798c4e6d-9915-4b7b-ad07-29a75fa24947"   # VER-94
...
def build_headline_comment(entry: dict) -> str:
    """Short CEO-facing summary comment for the report host issue."""
    date = entry["auditDate"]
    fleet = entry["fleet"]
    per_agent = entry.get("perAgent", [])
    ...
    lines = [
        ...
        "",
        f"- **Fleet synthetic spend:** ${fleet['estCostUsdDay']:.4f} "
        f"_(estimated, subscription token-derived — not billed)_",
        ...
        "",
        "Full report: [VER-94](/VER/issues/VER-94#document-morning-report) | "
...
def main() -> int:
    ...
    # 3. Render the comment
    comment_body = build_headline_comment(entry)
    ...
    # 5. Post a comment on the report host issue
    api(
        "POST",
        f"/api/issues/{args.report_issue_id}/comments",
        {"body": comment_body},
    )
    print(f"Comment posted on report host issue {args.report_issue_id}.")
    return 0
