...
/**
 * Stub global fetch to resolve to a Response-like object whose body streams the
 * given byte chunks in order — one chunk per reader.read() call. This lets a
 * single NDJSON line be split across reads to exercise the buffering logic.
 * Returns the fetch mock so a test can assert on the request arguments.
 */
...
/** Encode a list of frame strings as a single newline-joined byte chunk. */
...
describe("streamAiDraft", () => {
  ...
  it("reassembles a frame split across multiple reads", async () => {
    ...
    const cut = 8; // split mid-JSON, before any newline
    streamingResponse([
      bytes.slice(0, cut),
      bytes.slice(cut),
    ]);
  ...
  it("reassembles a multi-byte UTF-8 character split across reads", async () => {
    // Sinhala "ක" is 3 UTF-8 bytes; cut the delta string's character in half
    // across two reads. TextDecoder({stream:true}) must stitch it back.
    const line =
      JSON.stringify({ type: "description_delta", lang: "si", delta: "ක" }) +
      "\n";
    const bytes = encoder.encode(line);
    // Find a byte index that lands inside the multi-byte char's encoding.
    const cut = bytes.length - 4; // inside "ක" + closing quote/brace region
    streamingResponse([
      bytes.slice(0, cut),
      bytes.slice(cut),
    ]);
