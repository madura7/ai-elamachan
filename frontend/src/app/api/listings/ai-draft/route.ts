...
import type { Trilingual, Lang } from "@/lib/api/client";
import type { DraftStreamFrame, ListingDraft } from "@/lib/ai-draft";

/**
 * MOCK streaming AI-draft endpoint.
 *
 * Stands in for the real Go backend handler (VER-58). It honors the frozen
 * REST contract (multipart `photo?` + `keywords` in; `ListingDraft` out) and
 * emits NDJSON frames so the editor can render text as it forms.
 *
...
function mockTitle(keywords: string): Trilingual {
...
function mockDescription(keywords: string): Trilingual {
